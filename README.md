...
steps:
  ...
  # Install the `buf` CLI
  - uses: bufbuild/buf-setup-action@v0.9.10
...
|:---------------|:---------------------------------------------------|:-------------------|
| `version`      | The version of the [`buf` CLI][buf-cli] to install | [`v0.9.10`][version] |
...
steps:
  - uses: actions/checkout@v2
  # Installs version 0.9.10
  - uses: bufbuild/buf-setup-action@v0.9.10
    with:
      version: 0.9.10
  # Should output 0.9.10
  - run: buf --version
```
...
[version]: https://github.com/bufbuild/buf/releases/tag/v0.9.10
